...
This problem can be solved by a variant of the Bellman-Ford algorithm as described in~\cite{guerin2002computing, shufan2011two}.
...
We combine Step 1 and Step 2 in the following algorithm called $\KlmApprox$ (Algorithm~\ref{alg:optapprox}) that follows naturally from the two steps. Given $X$ and $\support(X)$ we add $x_0,x_{n+1}$ and construct the graph (line 2) as in Step 2. Then we execute a variant of the Bellman-Ford algorithm on $G$ for $m$ iterations to obtain a path $\vec{p}=(v_0,\dots,v_{m+1})$ (line 2) as described in Corollary \ref{cor:Bellman}. Finally we use Definition \ref{def:construction} to construct $X'$ from the weights of $\vec{p}$ (lines 4-5).
...
\begin{algorithm}\label{alg:optapprox}
	...
	Find a path $\vec{p}=(v_0,\dots,v_{m+1})$ such that $w(\vec{p})$ is minimal among all $\{w(\vec{p'}) \colon \vec{p'}\in paths_m(G, -\infty, \infty)\}$.   \;
...
\end{theorem}
\begin{proof}
	Constructing the graph $G$ as described in Step 2 takes $O(n^2)$ time and memory. Computing the shortest path can be achieved by the algorithm described in~\cite{guerin2002computing} in time time $O(n^2 m)$ and no additional memory allocation.
\end{proof}
...
\section{Discussion}\label{sec:discussion and future work}
