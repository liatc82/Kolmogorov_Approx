...
\begin{lemma}
Given any sequence $x_1 < x_2 <\cdots < x_{l}$ such that $x_1=-\infty$. $x_l=\infty$ and $x_i \in \support(X)$ for all $i{=}2,\dots,l-1$, there exists $X'$ such that $d_k(X,X') = \max\limits_{i=1,..,l-1} w(x_{i}, x_{i+1})$ and $P(x_{i} < X < x_{i+1}) = 0$ for all $i = 1,..,l-1$.
\end{lemma}
\begin{proof}
Define $X'$ by $f_{X'}(x_i) = w(x_{i-1},x_i) + w(x_i,x_{i+1}) + f_{X}(x_i)$ for $i=2,\dots,l-1$ and $f_{X'}(x)=0$ for $x \notin \{x_1,\dots,x_l\}$.
\end{proof}
