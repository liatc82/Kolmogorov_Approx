...
\end{lemma}
\begin{proof}
Since minimizing $d_K(X,X')$ subject to the constraints that $\support(X') \subseteq \support(X)$ and $|\support(X')|=m$ is a convex optimization problem, it has a solution $\varepsilon$. 
Let $X''$ be any random variable,  let $\{x_1,\dots,x_n\} = \support(X)$, and let $x_{n+1}=\infty$. Consider the the random variable $X'$ whose probability mass function is $f_{X'}(x_i) = P(x_i \leq X'' < x_{i+1})$ for $i=1,\dots,n$ and $F_{X'}(x)=0$ if $x\notin \support(X)$. By definition $d_{K}(X,X')=d_{K}(X,X'')$. Since $\support(X') \subseteq \support(X)$, we get that $d_{K}(X,X'') \geq \varepsilon$. Thus, $\varepsilon$ is a global minimum and any $X' \in \operatorname{argmin}\{ d_K(X,X')\colon \support(X') \subseteq \support(X), |\support(X')|=m \}$ is an $m$-optimal approximation of $X$.
\end{proof}
