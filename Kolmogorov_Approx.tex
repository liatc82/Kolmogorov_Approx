...
\begin{claim}
	For a random variable $X$, an ordered set $S=\{x_1<\dots<x_m\} \subset \support(X)$, $x_0=-\infty$ and $x_{n+1}=\infty$, there is no random variable $X'$ such that $\support(X')=S$ and $d(X,X') < \max\limits_{i=0,..,m} w(x_{i}, x_{i+1})$.
\end{claim}
\begin{proof}
	Let $i$ be the index that maximizes $w(x_{i}, x_{i+1})$. If $0<i<n-1$ then $d(X,X') \geq w(x_{i}, x_{i+1})$ by Lemma~\ref{lem:geq}. If $i=0$ or $i=n+1$ the same follows from Lemma~\ref{lem:geq1}.
\end{proof}


\begin{claim}
	For a random variable $X$, an ordered set $S=\{x_1<\dots<x_m\} \subset \support(X)$, $x_0=-\infty$ and $x_{n+1}=\infty$, there is a random variable $X'$ such that $\support(X')=S$ and $d(X,X')= \max\limits_{i=0,..,m} w(x_{i}, x_{i+1})$.
\end{claim}
\begin{proof}
