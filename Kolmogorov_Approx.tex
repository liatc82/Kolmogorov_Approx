...
\begin{lemma}
For $x_1< x_2 \leq x_3 < x_4 \leq \dots < x_{2l} \in \support(X) \cup \{-\infty,\infty\} $ there exists $X'$ such that $d_k(X,X') = \max\limits_{i=1 \dots l} w(x_{2i-1}, x_{2i})$ and $P(x_{2i-1} < X \leq x_{2i}) = 0, \forall i = 1 \dots l$.
\end{lemma}
\begin{proof}
	content...
\end{proof}
