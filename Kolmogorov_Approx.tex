...
Another related research is dealing with credit scoring~\cite{zeng2017comparison} where the main objective is to separate good applicants from bad applicants. The Kolmogorov–Smirnov statistic KS is a standard measure of model strength or model
performance in credit scoring. currently there are three computational methods of KS in terms of score binning: (1) the method with equal-width binning~\cite{mays2001handbook}, (2) the method with equal-size binning~\cite{refaat2011credit}, and (3) the method without binning~\cite{bolton2010logistic,siddiqi2012credit}, this method uses
a kind of binning in which each score is treated as a bin, or equal-width binning with a width of 0. Essentially, this work aims at presenting a comparison study of the three methods in 3 aspects: Values, Rank Ordering of Scores and Geometrical Way.%Their results indicate that among the three computational methods of KS, the method without binning always gives the largest value of KS. 
We would like to suggest our approximation method, the $\KlmApprox$ algorithm, described in this work as another comparison method which provides optimality guarantees that are not given in the binning technique.

The present study is also a continuation of the work of Pavlikov and Uryasev~\cite{pavlikov2016cvar}, where a procedure to produce a random variable $X'$ that optimally approximates a random variable $X$ is presented. Their approximation scheme, achieved using convex and linear programming, is designed for a different notion of distance (called CVaR). The new contribution of the present work in this context is that our method is direct, not using linear or convex programming, thus allowing tighter analysis of time and memory complexity. 
...
\begin{proof}
	...
	[[DF: will continue tomorrow, please don't touch]]
Let $X''$ be any random variable whose support is of size smaller or equal to $m$, we find a random variable $X'$ with $\support(X') \subseteq \support(X)$ and $d_{K}(X,X')=d_{K}(X,X'')$.
Let $\{x_1,\dots,x_n\} = \support(X)$, and let $x_0 = -\infty, x_{n+1}=\infty$. Consider the the random variable $X'$ whose probability mass function is
$f_{X'}(x_i) = P(x_{i-1} < X'' < x_i)$ for $i=1,\dots,n-1$,  $f_{X'}(x_n) = P(x_n-1 < X'' < x_{n+1})$ and $F_{X'}(x)=0$ if $x\notin \support(X)$. 
First see that for every $x_i<x<x_{i+1}$ for $i=0,\dots,n-1$ we have $|F_{X}(x)-F_{X'}(x)| \leq |F_{X}(x_i)-F_{X'}(x)|$  



By definition $d_{K}(X,X')=d_{K}(X,X'')$. Since $\support(X') \subseteq \support(X)$ and $|\support(X')|<|\support(X'')|\leq m$, we get that $d_{K}(X,X'') \geq \varepsilon$. Thus, $\varepsilon$ is a global minimum and any $X' \in \operatorname{argmin}\{ d_K(X,X')\colon \support(X') \subseteq \support(X), |\support(X')|\leq m \}$ is an $m$-optimal approximation of $X$.
\end{proof}
...
\begin{table}[th]
	...
	\begin{tabular}{|c|c|c|c|c|c|c|}
		...
		%N=10
		\multirow{3}{*}{Sequential}  & {2} & 0.0093 & 0.015 &  0.024 & 0 & 0.0009 \\ \Xcline{2-7}{1pt}  
