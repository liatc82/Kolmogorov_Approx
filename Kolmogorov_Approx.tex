...
In these terms, our problem can be phrased as:
$$
\min_{\alpha \in \mathbb{R}^p} \|x - D\alpha\| \text{ subject to }  \|\alpha\|_0 = m \text{ and } \|\alpha\|_1=1.
$$
where $D$ is the all-ones triangular matrix (the value at row $i$ and column $j$ is one if $i\leq j$ and zero otherwise), $x$ is related to $X$ such that the $i$th coordinate of $x$ is $F_X(x_i)$ where $\support(X)=\{x_1 < x_2 < \cdots < x_n\}$ and $\alpha$ is related to $X'$ such that the $i$th coordinate of $\alpha$ is $f_{X'}(x'_i)$ where $\support(X')=\{x'_1 < x'_2 < \cdots < x'_m\}$. The functions $F_X$ and $f_{X'}$ represent, receptively, the cumulative distribution function of $X$ and the mass distribution function of $X'$.
