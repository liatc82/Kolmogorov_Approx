...
\begin{definition}\label{def:kolapprox}
	For two discrete random variables $X_1$ and $X_2$, we say that $X_2$ is a \emph{Kolmogorov approximation} of $X_1$ with the parameters $\varepsilon$ and $m$, denoted by 
	$X_1 \preceq_{\varepsilon,m} X_2$, if $\sup_t |F_X(t) - F_{X'}(t)| \leq \varepsilon$, for all $t$, and if $|\support(X_2)| \leq m$. 
\end{definition}

\begin{definition}\label{def:optimalapprox}
	For a random variable $X$ and $m \in \mathbb{N}$, 
	let $\varepsilon^* = \varepsilon^* (X,m)=\inf\{\varepsilon \colon \exists X'. X \preceq_{\varepsilon,m} X'\}$ be the \emph{optimal approximation error} for $X$ with a random variable whose support is of size $m$. 
\end{definition}
...
\begin{proposition}
	For any random variable $X$ and an ordered set $S=\{x_1<\dots<x_m\} \subset \support(X)$ there is no random variable $X'$ such that $\support(X')=S$ and $d_k(X,X') < \max\limits_{i=0,\dots,m} w(x_{i}, x_{i+1})$ where, to simplify notations, we assume that $x_0=-\infty$ and $x_{n+1}=\infty$.
\end{proposition}
...
\begin{proposition}
	For any random variable $X$ and an ordered set $S=\{x_1<\dots<x_m\} \subset \support(X)$ there is a random variable $X'$ such that $\support(X')=S$ and $d_k(X,X') = \max\limits_{i=0,\dots,m} w(x_{i}, x_{i+1})$ where, to simplify notations, we assume that $x_0=-\infty$ and $x_{n+1}=\infty$.
\end{proposition}
\hl{$x_{n+1}=\infty$ n or m?}
\begin{proof}
